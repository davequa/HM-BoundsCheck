...
int test14(){
	...
	char *newbuffer = Dlib_malloc(9);

	if(checkMemoryAccess(newbuffer, 1) == 0){
		printf("CORRECT: IN-BOUNDS ACCESS DETECTED.\n");
	}else{
		printf("INCORRECT: IN-BOUNDS ACCESS DETECTED AS OUT-OF-BOUNDS.\n");
		Dlib_free(newbuffer);
	...
	//changed to acommodate for rounding up to 16 from 9.
	test = newbuffer + 16;
...
int test13(){
	//tests altered to acommodate for automatic conversion to size adhering to power of two.
	char *buffer;
	size_t sz = 62;
...
int test10(){
	...
	//test altered to keep padding in mind 
	char *test = buffer;
	test = test + 16384 - 64;
...
int test2(){
	...
	//test changed, because 48 -> 64 rounded.
	test = new + 64;
...
void testm(){
	...
	printf("REMINDER: SHADOW BYTE MAPPING TEST:\n");
	...
	printf("REMINDER: SHADOW MAPPING BYTE TEST COMPLETED.\n\n");
...
int main(int argc, char **argv){
	...
	printf("Test 0: BASIC SHADOW MEMORY TEST\n");
	if(test0() == 0){
...
/*	if(test11() == 0){
	...
	printf("WARNING: NOT INSTRUMENTED!\n");
	printf("Test 11: SKIPPED.\n");
	...
	printf("Test 13: UNALIGNED SHADOW MEMORY TEST\n");
	if(test13() == 0){
	...
	//Only activate if alternative registration method is active.
	printf("Test 14: SPECIAL MEMORY ALTERNATIVE TEST\n");
	if(test14() == 0){
	...
	printf("Test 16: ADDITIONAL TEST FOR STRANGE BUG\n");
	if(test16() == 0){
