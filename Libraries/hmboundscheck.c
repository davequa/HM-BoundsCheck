...
int getLock(size_t sz){
	...
	if(sz == 8){
		...
		return 7;
	}else if(sz > 1024 && sz <= 4096){
		return 8;
	}else if(sz > 4096 && sz <= 8192){
		return 9;
	}else if(sz > 8192){
		return 10;
	}
...
int unlock(size_t sz){
	...
	lock = getLock(sz);
	if(lock == -1){
		printf("ERROR: SIZE PROVIDED WAS INVALID.\n");
		return 1;
	}
...
int lock(size_t sz){
	...
	do{
		...
		if(ifLocked == EINVAL){
			printf("ERROR: MUTEX WAS INVALID.\n");
			return 1;
	...
	if(ASANCheckInit == 0){
		...
		if(var != 0){
			...
			if(((unsigned long long int) mem & 7) + accessSize > var){
				...
				/* If no usable value was read, it means the shadow memory contained garbage. Therefore, we simply treat this
				   access as an access that was too far out-of-bounds. */
				return 1;
			}
		}else{
			if(accessSize > 1){
...
int removeAddr(void *memL, void *memR){
	...
	if(memset((unsigned char*) shadowAddr, 64, ((2 * toWriteRZ) + toWriteAM)) == NULL){
		return 1;
	}
...
int registerAddr(void *memL, void *memR){
	...
	if(shadowAddrL == NULL || shadowAddrR == NULL || shadowAddrRegion == NULL){
		return 1;
	}
	...
	if(memset((unsigned char*) shadowAddrL, (unsigned char) 0xFF, toWriteRZ) == NULL){
		return 1;
	}

	/* Write the shadow memory of the right red-zone. */
	if(memset((unsigned char*) shadowAddrR, (unsigned char) 0xFF, toWriteRZ) == NULL){
		return 1;
	}

	/* Write the shadow memory of the addressable memory region. */
	if(memset((unsigned char*) shadowAddrRegion, (unsigned char) 0, toWriteAM) == NULL){
		return 1;
	}

	if(ASANCheckInit == 0){
		if(sz_rem != 0){
			if(memset((unsigned char*) (shadowAddrRegion + toWriteAM), (unsigned char) sz_rem, 1) == NULL){
				return 1;
			}
		}
	}else{
		if(sz_rem != 0){
			...
			if(memset((unsigned char*) (shadowAddrRegion + toWriteAM), avar, 1) == NULL){
				return 1;
			}
		}
	}
...
memSeg *createBlock(void *start, size_t originalsz){
	...
	//Set all addresses and flags correctly.
	new->next = NULL;
...
int returnBlockToFreeList(void *mem, int startAdd){
	...
	szgetter--;
	allocation_sz = *(size_t*) szgetter;	//fix
	...
	if(startAdd == 0){
		if(removeAddr(new->startAddrL, new->startAddrR) == 1){
			free(new);
			return -1;
		}
	}
...
void *getBlockFromFreeList(size_t sz){
	...
	if(registerAddr(toReturn->startAddrL, toReturn->startAddrR) == 1){
		free(toReturn);
		return NULL;
	}
...
int setFreeList(void *blockStart, int index, size_t sz){
	...
	for(size_t x = standardPreAllocSize; x > 0; x--){
		...
		if(memmove(szsetter, &sz, sizeof(size_t)) == NULL){
			return 1;
		}
...
int allocateFreeList(size_t sz){
	...
	if(setFreeList(blockStart, index, sz) == 1){
		return 1;
	}
...
void *Dlib_malloc(size_t sz){
	...
	if(useFreeLists == 0 && useRegistration == 0){
		...
		if(check == 2){
			/* A new free list must be allocated for a piece of this memory. */
	//		if(lock(sz) == 1){
	//			return NULL;
	//		}
